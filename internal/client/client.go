...
// 该函数向Ping0.cc发送初始请求，并从响应中提取x1参数和JavaScript路径，
// 这两个参数对于后续请求是必需的。
//
// 返回:
...
func GetInitialPage() (string, string, error) {
	...
	if constants.ManualX1Value != "" {
		...
		return constants.ManualX1Value, "/js/main.js", nil
	}
	...
	doc.Find("script").Each(func(i int, s *goquery.Selection) {
		content := s.Text()
		if strings.Contains(content, "window.x1") || strings.Contains(content, "var x1") {
			// 使用字符串函数提取x1值
	...
	return x1Value, jsPath, nil
}

// GetFinalPage 获取最终页面
func GetFinalPage(key string) (string, error) {
	// 创建带超时的上下文
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	if constants.Verbose {
		log.Printf("设置Cookie: js1key=%s", key)
